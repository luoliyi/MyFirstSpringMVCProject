...
@Controller
@RequestMapping(path = {"/helloservlet2","/HelloServlet2","/servlet2","/Servlet2"},method =RequestMethod.GET) //defalut:GET
public class HelloServlet2 { //c：调用业务逻辑，取得数据，转发页面
    @RequestMapping("")
    public String hello(Model model) {
        model.addAttribute("message", "/helloservlet2"); //m：业务逻辑，保持数据状态
        return "hi"; //v：显示
    }
    @RequestMapping(path = "/show2",method = RequestMethod.GET) //defalut:GET
    public String show(Model model) {
        model.addAttribute("message", "/helloservlet2/show2!");
        return "show";
    }
}